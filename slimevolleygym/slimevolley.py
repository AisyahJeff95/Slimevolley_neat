...
def setNightColors():
  ...
  global BACKGROUND_COLOR, FENCE_COLOR, COIN_COLOR, GROUND_COLOR, GROUNDRIGHT_COLOR, GROUNDLEFT_COLOR
  BALL_COLOR = (217, 79, 0)
  AGENT_LEFT_COLOR = (35, 93, 188)
  AGENT_RIGHT_COLOR = (255, 165, 0)  # 255, 165, 0
  ...
  GROUNDRIGHT_COLOR = (255, 192, 203)
  GROUNDLEFT_COLOR = (160, 32, 240)
...
class Agent:
  ...
  def __init__(self, dir, x, y, c):
    ...
    self.desired_vx = 0
    self.desired_vy = 0
    self.current_action = None  # Initialize current_action here
  ...
  def setAction(self, action):
    ...
    if (forward and (not backward)):
      ...
      self.current_action = 'forward' # ecah

    if (backward and (not forward)):
      self.desired_vx = PLAYER_SPEED_X
      self.current_action = 'backward' # ecah

    if jump:
      self.desired_vy = PLAYER_SPEED_Y
      self.current_action = 'jump' # ecah

    # # Handle jumping (ecah)
    # if jump:
    #     self.desired_vy = PLAYER_SPEED_Y
    #     # If jumping, we can keep the current_action as is or set it to 'jump'
    #     if self.current_action is None:  # Only set if not already moving
    #         self.current_action = 'jump'
    # # If neither forward nor backward nor jump, set to None
    # if not forward and not backward and not jump:
    #     self.current_action = None  # No movement at all
...
class Game:
    ...
    def __init__(self, np_random=np.random):
        ...
        self.reset()
        self.time_on_right = 0  # Track time the ball is on the right side
        self.reward_for_left = 0  # Reward for the ball being on the left side

    def reset(self):
        self.ground = Wall(0, 0.80, REF_W, REF_U, c=GROUND_COLOR)
        self.ground_left = Wall(-12, 0.8, -REF_W / 2, REF_U, c=GROUNDLEFT_COLOR)
        self.ground_right = Wall(12, 0.8, REF_W / 2, REF_U, c=GROUNDRIGHT_COLOR)
        ...
        self.time_on_right = 0  # Reset the time tracker
        self.reward_for_left = 0  # Reset the reward tracker
    ...
    def step(self):
        ...
        # Agent return ball to opponent side (left side) - ecah
        if self.ball.isColliding(self.agent_right):
            self.ball.bounce(self.agent_right)
            # total_reward += 0.5  # Reward for successfully hitting the ball
            # print("right hit ball")
            if self.ball.vx < 0:  # If the ball is moving towards the left side
                # print("ball go left")
                total_reward += 1.0 # Reward for returning the ball
            else:
                total_reward -= 0.5 # penalty for not giving ball to opponent

        # Ball bounce on fence and go to opponent side - ecah
        if self.ball.isColliding(self.fenceStub):
            self.ball.bounce(self.fenceStub)
            if self.ball.vx < 0:
                total_reward += 1.0 # add reward here, who knows might be good strategy 
                
        # Update the agents after setting their actions
        self.agent_right.update()

        # negated (-ve value), since we want reward to be from the perspective of right agent being trained.
        ...
        if result != 0:
          ...
          if result < 0:  # Baseline agent won
            ...
            total_reward -= 2.0 # reward is higher here to emphasize the importance - ecah
          else:
            self.agent_left.emotion = "sad"
            self.agent_right.emotion = "happy"
            self.agent_left.life -= 1
            # print("opponent lose 1 life")
            total_reward += 2.0
          return total_reward + result  # Return the total reward plus the game result
    ...
    def display(self, canvas):
        ...
        canvas = self.ground_right.display(canvas)
        canvas = self.ground_left.display(canvas)
        return canvas
...
class FrameStack(gym.Wrapper):
  def __init__(self, env, n_frames):
    ...
    # Handle different observation space shapes (ecah)
    if len(shp) == 1:  # If it's a 1D array
        self.observation_space = gym.spaces.Box(
            low=-np.inf, high=np.inf, shape=(n_frames, shp[0]), dtype=env.observation_space.dtype
        )
    elif len(shp) == 2:  # If it's (height, width)
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(shp[0], shp[1], n_frames), dtype=env.observation_space.dtype
        )
    elif len(shp) == 3:  # If it's (height, width, channels)
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(shp[0], shp[1], shp[2] * n_frames), dtype=env.observation_space.dtype
        )
    else:
        raise ValueError("Unsupported observation space shape: {}".format(shp))
  ...
  def _get_ob(self):
      ...
      return np.stack(list(self.frames), axis=0)  # Stack along a new dimension (ecah)
